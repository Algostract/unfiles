...
<template>
    ...
    <div class="relative aspect-square w-full overflow-hidden">
      ...
      <div
        ...
        <div class="pointer-events-auto flex flex-col gap-0.5 text-xs">
          <span class="flex items-center gap-1 truncate"> <NuxtIcon name="lucide:file" size="14" /> {{ baseName }} </span>
          <div class="flex justify-between gap-6">
            <span class="flex items-center gap-1"> <NuxtIcon name="lucide:database" size="14" /> {{ formatedSize }} </span>
        ...
        <div class="pointer-events-auto flex gap-2">
          ...
          <NuxtLink
            ...
            :to="`/media/${media.name}/w_1024&q_80`"
            target="_blank"
  ...
  <div v-else class="relative flex items-center overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm transition hover:shadow-lg dark:border-neutral-700 dark:bg-neutral-800">
    <img ref="img" class="m-2 h-16 w-16 rounded-md object-cover" :src="media.thumbnail!" alt="thumbnail" @load="handleImgLoad" />
    <div class="flex min-w-0 flex-1 flex-col gap-1">
      <div class="flex items-center gap-2">
        <span class="font-medium flex items-center gap-1 truncate"> <NuxtIcon name="lucide:document" size="14" /> {{ media.name }} </span>
        <NuxtLink v-if="media.original" :to="media.original" external target="_blank" class="ml-auto flex items-center gap-1 rounded bg-white/20 p-1 backdrop-blur hover:bg-white/30">
          <NuxtIcon name="lucide:eye" size="14" />
        </NuxtLink>
        <button class="flex items-center gap-1 rounded bg-white/20 p-1 backdrop-blur hover:bg-white/30" @click.stop="emit('refetch', media.id)">
          <NuxtIcon name="lucide:refresh-cw" size="14" />
        </button>
      </div>
