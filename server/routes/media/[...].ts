...
const mime: Record<string, string> = {
	...
	png: 'image/png', gif: 'image/gif', heif: 'image/heif', tiff: 'image/tiff', svg: 'image/svg+xml',
}
...
const syncDrive = defineCachedFunction(async () => {
...
function normalizeArgs(rawArgs: string) {
	const decodedArgs = decodeURIComponent(rawArgs || '')
		...
		.replace(/&/g, ',')
		.replace(/\s+/g, '')
	const tokens = decodedArgs.split(',').filter(Boolean).sort((a, b) => a.localeCompare(b))
